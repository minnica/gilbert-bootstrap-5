$(document).ready(function() {
    ...
    tablaUsuarios = $('#tablaUsuarios').DataTable({
        ...
        "columns": [{
            ...
            "data": "fecha_revision_corte"
        }, {
            "data": "status"
        }, {
            "data": "comentarios"
        }, {
            "data": "revision_entrega"
        }, {
            "data": "fecha_revision_entrega"
        }, {
    ...
    $('#formUsuarios').submit(function(e) {
        ...
        revision_corte = $.trim($('#revision_corte').val());
        fecha_revision_corte = $.trim($('#fecha_revision_corte').val());
        status = $.trim($('#status').val());
        comentarios = $.trim($('#comentarios').val());
        revision_entrega = $.trim($('#revision_entrega').val());
        fecha_revision_entrega = $.trim($('#fecha_revision_entrega').val());
        ...
        $.ajax({
            ...
            data: {
                ...
                revision_corte: revision_corte,
                fecha_revision_corte: fecha_revision_corte,
                status: status,
                comentarios: comentarios,
                revision_entrega: revision_entrega,
                fecha_revision_entrega: fecha_revision_entrega,
                opcion: opcion
            },
    ...
    $(document).on("click", ".btnEditar", function() {
        ...
        revision_corte = fila.find('td:eq(15)').text();
        fecha_revision_corte = fila.find('td:eq(16)').text();
        status = fila.find('td:eq(17)').text();
        comentarios = fila.find('td:eq(18)').text();
        revision_entrega = fila.find('td:eq(19)').text();
        fecha_revision_entrega = fila.find('td:eq(20)').text();
        ...
        $("#revision_corte").val(revision_corte);
        $("#fecha_revision_corte").val(fecha_revision_corte);
        $("#status").val(status);
        $("#comentarios").val(comentarios);
        $("#revision_entrega").val(revision_entrega);
        $("#fecha_revision_entrega").val(fecha_revision_entrega);
